...
def inject_company_link_nav() -> None:
    ...
    Markdown ``<script>`` tags are unreliable under Streamlit, and plain ``<a>``
    clicks are intercepted inside the nested embed iframe. ``components.html``
    runs real JS in a child frame; from there the app document is
    ``window.parent`` and the marketslite embed shell is ``window.parent.parent``.
    """
    components.html(
        """
        <script>
