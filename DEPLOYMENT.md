...
| Data stamp looks days old | Scheduled Action failed to commit, Streamlit hasn’t redeployed yet, or a CDN cached `site/data-status.json` | Check **Actions → Earnings Data Refresh** for green runs + an `Automated data refresh` commit on `main`; landing prefers GitHub’s `data-status.json`, app reads the same file from the deploy |
